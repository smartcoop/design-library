...
{
    /// <summary>
    ///     Abstract base class <see cref="ITagHelper" /> to render smart design form groups with asp-for, helper-text , label,
    ...
    public abstract class BaseSmartFormGroupTagHelper : BaseTagHelper
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(Id)) Id = context.UniqueId;

            if (!string.IsNullOrWhiteSpace(For?.Name))
                Name = For.Name;
            else if (string.IsNullOrWhiteSpace(Name)) Name = Id;
        }
