...
namespace Smart.Design.Razor.TagHelpers.Html
{
    public class SmartHtmlGenerator : ISmartHtmlGenerator
    {
        ...
        /// <inheritdoc />
        public TagBuilder GenerateSmartInputText(ViewContext viewContext, string id, string name, string placeholder,
            object? value, ModelExpression @for)
        {
            ...
            if (!string.IsNullOrWhiteSpace(id)) inputTextTagBuilder.Attributes[id] = id;

            if (!string.IsNullOrWhiteSpace(inputName)) inputTextTagBuilder.Attributes["name"] = inputName;

            if (!string.IsNullOrWhiteSpace(placeholder)) inputTextTagBuilder.Attributes["placeholder"] = placeholder;
        ...
        /// <inheritdoc />
        public TagBuilder GenerateFormGroup(string id, string name, string label, string helperText,
            TagBuilder controls)
        {
            ...
            if (controls is not null) formGroupControlsContainer.InnerHtml.AppendHtml(controls);
        ...
        public TagBuilder GenerateFormGroupLabel(string label, string labelFor)
        {
            ...
            if (!string.IsNullOrWhiteSpace(label)) labelTagBuilder.InnerHtml.Append(label);

            if (!string.IsNullOrWhiteSpace(labelFor)) labelTagBuilder.Attributes.Add("for", labelFor);
        ...
        public TagBuilder GenerateSmartRadio(string id, string name, string label, string value, bool @checked,
            ModelExpression @for)
        {
            ...
            if (@checked) inputRadioTagBuilder.Attributes.Add("checked", "checked");
            ...
            else if (!string.IsNullOrWhiteSpace(value)) inputRadioTagBuilder.Attributes.Add("value", value);
        ...
        public virtual TagBuilder GenerateSmartPanel(string header, IHtmlContent content)
        {
            ...
            if (!string.IsNullOrEmpty(header)) headerTagBuilder.InnerHtml.SetContent(header);
        ...
        /// <inheritdoc />
        public TagBuilder GenerateSmartTextArea(string id, string name, TagHelperContent directContent,
            int? rows = default, TextAreaSize textareaSize = TextAreaSize.Unspecified, ModelExpression @for = default)
        {
            var textAreaTagBuilder = new TagBuilder("textarea");
            textAreaTagBuilder.AddCssClass("c-textarea");

            if (!string.IsNullOrWhiteSpace(id)) textAreaTagBuilder.Attributes.Add("id", id);
            ...
            else if (!directContent.IsEmptyOrWhiteSpace) textAreaTagBuilder.InnerHtml.SetHtmlContent(directContent);
        ...
        public TagBuilder GenerateSmartButtonToolbar(ButtonToolbarLayout layout, bool isCompact)
        {
            ...
            if (layout == ButtonToolbarLayout.Vertical) toolbar.AddCssClass("c-button-toolbar--vertical");

            if (isCompact) toolbar.AddCssClass("c-button-toolbar--compact");
        ...
        /// <summary>
        ///     Sets <see cref="TagBuilder" />'s name depending on the value of <paramref name="modelExpression" /> and
        ///     <paramref name="name" />.
        ///     If both <paramref name="modelExpression" /> <see cref="ModelExpression.Name" /> and <paramref name="name" /> are
        ///     empty or null, the attribute is not set.
        ///     <see cref="ModelExpression.Name" /> of <paramref name="modelExpression" /> property has precedence over
        ///     <paramref name="name" />.
        /// </summary>
        /// <param name="tagBuilder">The <see cref="TagBuilder" /> to which attribute name should be set.</param>
        /// <param name="modelExpression">
        ///     A <see cref="ModelExpression" /> that can be associated to the
        ///     <paramref name="tagBuilder" />
        /// </param>
        ...
        private void AddNameAttribute(TagBuilder tagBuilder, ModelExpression modelExpression, string name)
        {
            ...
            if (!string.IsNullOrWhiteSpace(attributeName)) tagBuilder.Attributes["name"] = attributeName;
        }
    }
}
